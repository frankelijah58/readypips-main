...
export default function SubscriptionManagement({ admin }: { admin: any }) {
  ...
  const normalizeProvider = (provider?: string) => {
    return String(provider || '').replace(/"/g, '').trim().toLowerCase();
  };

  const getCurrencyByProvider = (provider?: string) => {
  ...
  const formatAmount = (amount: number | string, provider?: string) => {
    const numericAmount = Number(amount || 0);
    const currency = getCurrencyByProvider(provider);
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === subscriptions.length && subscriptions.length > 0) {
      setSelectedIds([]);
    } else {
      ...
      <section className="space-y-6 transition-all duration-300">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            >
              <ShieldAlert
                className={`w-5 h-5 ${isQueueOpen ? 'text-amber-600' : 'text-slate-400'}`}
              />
            </div>
            <div>
              ...
              <tbody className="divide-y divide-slate-50">
                {subscriptions.map((sub) => (
                  <tr key={sub._id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <PaymentModeBadge provider={sub.plan} />
                    </td>
                    ...
                    <td className="px-6 py-4 text-xs text-slate-600">
                      <div className="flex items-center gap-2">
                        ...
                        {sub.updatedDate ? new Date(sub.updatedDate).toLocaleDateString() : '-'}
                      </div>
                    </td>

                    <td className="px-6 py-4 text-right font-mono font-bold text-slate-700">
                      {formatAmount(sub.price, sub.provider)}
                    </td>
...
function PendingCard({ pay, onAction, processingId, formatAmount }: any) {
  return (
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase">
              {String(pay.provider || '').replace(/"/g, '')}
            </p>
          </div>
        </div>
        <span className="text-sm font-black text-indigo-600">
          {formatAmount(pay.amount, pay.provider)}
        </span>
      </div>
...
function PaymentModeBadge({ provider }: { provider?: string }) {
  const value = String(provider || '').replace(/"/g, '').trim().toLowerCase();

  const styles: Record<string, string> = {
    "mpesa": 'bg-green-50 text-green-700 border-green-100',
    'm-pesa': 'bg-green-50 text-green-700 border-green-100',
    stripe: 'bg-violet-50 text-violet-700 border-violet-100',
    paypal: 'bg-blue-50 text-blue-700 border-blue-100',
    binance: 'bg-yellow-50 text-yellow-700 border-yellow-100',
    whop: 'bg-indigo-50 text-indigo-700 border-indigo-100',
  ...
  return (
    <span
      className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase border ${
        styles[value] || 'bg-slate-50 text-slate-700 border-slate-100'
      }`}
    >
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase border ${
        styles[normalizedStatus] || 'bg-slate-50 text-slate-700 border-slate-100'
      }`}
    >
